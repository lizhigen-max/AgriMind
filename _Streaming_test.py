...
async def simple_streaming_demo(user_id, question):
    # 用户ID
...
async def main():
    ...
    try:
        import time
        t1 = time.time()
        await simple_streaming_demo('uuid-001', '葡萄冬季修剪应该注意什么')
        t2 = time.time()
        await simple_streaming_demo('uuid-002', '葡萄叶子发黄是什么原因')
        print(f'耗时：{t2 - t1}')
        print(f'耗时：{time.time() - t1}')

        # await asyncio.gather(simple_streaming_demo('uuid-001', '葡萄冬季修剪应该注意什么'),
        #                                               simple_streaming_demo('uuid-002', '葡萄叶子发黄是什么原因'))
        # print(f'耗时：{time.time() - t1}')
